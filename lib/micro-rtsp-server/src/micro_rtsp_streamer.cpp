...
uint8_t *micro_rtsp_streamer::create_jpg_packet(const uint8_t *jpg_scan, const uint8_t *jpg_scan_end, uint8_t **jpg_offset, const uint32_t timestamp, const uint8_t *quant_lum, const uint8_t *quant_chr, size_t &packet_size)
{
    ...
    // Number of JPEG scan bytes in this packet, kept below the Ethernet MTU so a RTP/UDP datagram is never fragmented.
    ...
    const size_t header_size = rtp_over_tcp_hdr_size + rtp_hdr_size + jpeg_hdr_size + (include_quantization_tables ? (jpeg_qtable_hdr_size + 2 * jpeg_qtable_size) : 0);
