...
// RTP over RTSP/TCP framing header (RFC 2326 section 10.12). This 4 byte header is prepended to every RTP packet when the transport is "RTP/AVP/TCP" // (interleaved).
// The length field is the number of bytes of the encapsulated RTP packet, in network byte order.
...
// Maximum number of JPEG scan bytes carried in a single RTP packet. Kept well below the 1500 byte Ethernet MTU so RTP/UDP datagrams are never fragmented.
